...
def render_detail_page(entry: dict, lang: str) -> str:
    """
    Detail page renderer (NO TRANSLATION).
    - KO detail: title uses name_ko if present, else name_en/name/_id.
      If both KO and EN exist, show EN as a small meta line.
    - EN detail: title uses name_en (fallback name/_id). No Korean/translation meta on EN pages.
    """
    item_id = entry.get("_id") or ""
    name_en = entry.get("name_en") or entry.get("name") or item_id
    name_ko = entry.get("name_ko") or ""

    if lang == "ko":
        title_name = name_ko or name_en or item_id
        subtitle_html = f"<div class='meta'>{esc(name_en)}</div>" if (name_ko and name_en) else ""
    else:  # lang == "en"
        title_name = name_en or entry.get("name") or item_id
        subtitle_html = ""

    url = entry.get("url") or ""
    version = entry.get("version") or ""
    last_updated = entry.get("last_updated") or ""
    one_line = entry.get(f"one_line_{lang}") or entry.get("one_line") or entry.get("one_line_en") or entry.get("one_line_ko") or ""
    ...
    funcs_html = "".join([f"<li>{esc(x)}</li>" for x in functions]) if functions else ""
    tags_html = " ".join([f"<span class='tag'>{esc(x)}</span>" for x in tags]) if tags else ""

    # Back-to-list handling via query param (works on GH Pages static)
    back_script = """
    <script>
      (function(){
        ...
        if(back){
          ...
          if(a){ a.href = back; a.style.display='inline-block'; }
        }
      })();
    </script>
    """
    ...
    pills = []
    pills.append("<a id='backLink' class='pill' href='#' style='display:none'>← 목록(티스토리)로</a>")
    if github_url:
        pills.append(f"<a class='pill' href='{esc(github_url)}' target='_blank' rel='noopener noreferrer'>GitHub</a>")
    if tistory_url:
        pills.append(f"<a class='pill' href='{esc(tistory_url)}' target='_blank' rel='noopener noreferrer'>티스토리</a>")
    if url:
        pills.append(f"<a class='pill' href='{esc(url)}' target='_blank' rel='noopener noreferrer'>GPT 바로가기</a>")
...
<style>
  ...
  .meta {{ color: #666; margin: 4px 0 16px; font-size: 14px; }}
  .pill {{ font-size: 12px; color: #333; background: #f3f3f3; padding: 6px 10px; border-radius: 999px; text-decoration: none; margin-right: 6px; display: inline-block; }}
  .tag {{ display:inline-block; background:#f6f6f6; border-radius:999px; padding:4px 10px; margin:2px 6px 2px 0; font-size:12px; }}
...
<body>
  <div class="wrap">
    ...
    <div class="card">
      ...
      <div><strong>Last Updated:</strong> {esc(last_updated)}</div>
      <div style="margin-top:10px;"><strong>One-line:</strong> {esc(one_line)}</div>
      {"<div style='margin-top:10px;'><strong>Tags:</strong> "+tags_html+"</div>" if tags_html else ""}
    </div>

    {"<div class='card'><strong>Functions</strong><ul>"+funcs_html+"</ul></div>" if funcs_html else ""}
...
def render_index(entries, lang: str, title: str, details_prefix: str, extra_link_html: str, tistory_list_url: str) -> str:
    ...
    for idx, e in enumerate(entries, start=1):
        ...
        # Display rules (NO TRANSLATION):
        # - KO index: show existing Korean and English if both exist: "KO (EN)".
        #            if only one exists, show that one.
        # - EN index: show English if available; otherwise fall back to name/title/id.
        if lang == "ko":
            if name_ko and name_en:
                display_name = f"{name_ko} ({name_en})"
            else:
                display_name = name_ko or name_en or item_id
        else:  # lang == "en"
            display_name = name_en or e.get("name") or item_id
        ...
        btns = []
        if gpt_url:
            btns.append(f"<a class='btn btn-small' href='{esc(gpt_url)}' target='_blank' rel='noopener noreferrer'>GPT 바로가기</a>")
        btns.append(f"<a class='btn btn-small' href='{detail_href}'>상세정보</a>")
        if tistory_url:
            btns.append(f"<a class='btn btn-small' href='{esc(tistory_url)}' target='_blank' rel='noopener noreferrer'>티스토리</a>")
        if github_url:
            btns.append(f"<a class='btn btn-small' href='{esc(github_url)}' target='_blank' rel='noopener noreferrer'>GitHub</a>")
...
def build():
    ...
    # ensure name_ko_policy is set when name_ko is provided (NO AUTO TRANSLATION)
    for e in all_loaded:
        if e.get("name_ko") and not e.get("name_ko_policy"):
