...
def guess_lang(entry: dict, filename: str) -> str:
    ...
    Supports language tokens anywhere in the filename, e.g.
      - something_en_title.yaml
      - something-kr-v2.yml
      - en_something.yaml

    Token rule: language must appear as its own segment separated by '_' or '-',
    or at the start/end of the base name.
    """
    fn = (filename or "").lower()
    base = re.sub(r"\.(yaml|yml)$", "", fn)

    # token-based match (safer than substring; avoids accidental matches like 'green')
    # accepts: en, eng, english / ko, kr, kor, korean
