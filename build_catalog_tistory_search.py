...
def insert_search_box(page_html: str, lang: str) -> str:
...
'''
    ...
    return page_html.replace("</h1>", "</h1>" + script, 1)
...
def _display_name_primary_for_ko(e: dict) -> str:
...
def sort_key_ko_index(e: dict):
    primary = _display_name_primary_for_ko(e)
    is_hangul = bool(_HANGUL_START_RE.match(primary))
    if is_hangul:
        return (0, primary, (e.get("_id") or ""))
    return (1, primary.lower(), (e.get("_id") or ""))
...
# Sort indexes (KO: Hangul first 가나다순, then English A–Z)
