...
def render_detail_page(entry: dict, lang: str) -> str:
    ...
    if lang == "ko":
        # KO detail display rules:
        # - If original Korean exists -> Korean title only (no English subtitle).
        # - If Korean is auto-generated from English -> English title, show auto Korean as meta.
        # - If only English exists -> English title.
        if name_ko and ko_policy != "auto":
            title_name = name_ko
            subtitle_html = ""
        elif name_ko and ko_policy == "auto":
            title_name = name_en or name_ko or entry.get("name") or ""
            subtitle_html = f"<div class='meta'>(자동번역) Korean: {esc(name_ko)}</div>" if name_ko else "<div class='meta'>(자동번역)</div>"
        else:
        # EN detail display rules:
        # - Show English title when available; otherwise show Korean title only.
        title_name = name_en or name_ko or entry.get("name") or ""
        if name_en and name_ko:
            if ko_policy == "auto":
                subtitle_html = f"<div class='meta'>(자동번역) Korean: {esc(name_ko)}</div>"
            else:
                subtitle_html = f"<div class='meta'>Korean: {esc(name_ko)}</div>"
        else:
            subtitle_html = ""
        one_line = (
