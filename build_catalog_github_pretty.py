    ...
    <script>
      ...
      function filterList(){
        ...
        items.forEach(function(li){
          var hay = norm(li.getAttribute("data-name")) + " " + norm(li.getAttribute("data-tags")) + " " + norm(li.getAttribute("data-one"));
...
def detail_page(e: dict, lang: str) -> str:
    ...
    for x in funcs:
        if not x:
            continue
        func_items += f"<li class='fitem'>{esc(x)}</li>"
...
def build(clean: bool = True):
    ...
    with open(root_index, "w", encoding="utf-8") as f:
            f.write(
                ...
                f"{'<span class=\"badge\">제한공개</span>' if restricted else ''}"
                f"</div>"
                f"<div class='meta'>{one}</div>"
                f"<div class='btnbar'>"
                f"{url_btn}"
                f"{(detail_btn if not restricted else '<span class=\"badge\">제한공개</span>')}"
                f"</div>"
                f"</div>"
            )
    ...
    with open(en_index, "w", encoding="utf-8") as f:
            f.write(
                ...
                f"<div class='title'>"
                f"<a class='titlelink' href='{title_href}'><div class='name'>{i}. {name}</div></a>"
                f"{'<span class=\"badge\">Restricted</span>' if restricted else ''}"
                f"</div>"
                f"<div class='meta'>{one}</div>"
                f"<div class='btnbar'>"
                f"{url_btn}"
                f"{(detail_btn if not restricted else '<span class=\"badge\">Restricted</span>')}"
                f"</div>"
                f"</div>"
            )
