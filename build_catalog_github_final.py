...
def guess_lang(entry: dict, filename: str) -> str:
    ...
    if (
        base.endswith("_en") or base.endswith("-en") or "_en_" in base or "-en-" in base
        or fn.endswith(("_en.yaml", "_en.yml")) or fn.endswith(("-en.yaml", "-en.yml"))
    ):
        return "en"

    if (
        base.endswith(("_ko", "_kr", "_kor")) or base.endswith(("-ko", "-kr", "-kor"))
        or any(tok in base for tok in ("_ko_", "_kr_", "_kor_", "-ko-", "-kr-", "-kor-"))
        or fn.endswith(("_ko.yaml", "_ko.yml", "_kr.yaml", "_kr.yml", "_kor.yaml", "_kor.yml"))
        or fn.endswith(("-ko.yaml", "-ko.yml", "-kr.yaml", "-kr.yml", "-kor.yaml", "-kor.yml"))
    ):
        return "ko"
...
def detail_page(e: dict, lang: str) -> str:
    is_ko = (lang == "ko")
    title = display_name_ko(e) if is_ko else display_name_en(e)
    back_href = make_back_link_from_detail(lang)
    back_label = "← 목록" if is_ko else "← Catalog"
    ...
    if isinstance(funcs, str):
        funcs = [funcs]
    func_html = "".join(f"<li>{esc(x)}</li>" for x in funcs)
    ...
    notice = ""
    if restricted:
        notice = "<p style='padding:10px;border-radius:10px;background:#fff3cd;color:#7a5a00;'>"
        notice += ("제한공개 항목입니다.</p>" if is_ko else "Restricted item.</p>")

    gpt_label = "GPT 바로가기" if is_ko else "Open GPT"
...
<body>
  <div class="topbar">
    ...
    {("<a class='btn' href='"+gpt_url+"' target='_blank' rel='noopener noreferrer'>"+gpt_label+"</a>") if (gpt_url and show_url) else ""}
    <a class="btn" href="{esc(TISTORY_BACK_DEFAULT)}" target="_blank" rel="noopener noreferrer">Tistory</a>
  </div>
  ...
  <h3>{"상세정보" if is_ko else "Details"}</h3>
  <ul>
    <li><strong>_id</strong>: <code>{esc(e.get("_id",""))}</code></li>
    <li><strong>gpt_id</strong>: {esc(e.get("gpt_id",""))}</li>
    <li><strong>url</strong>: {gpt_url}</li>
    <li><strong>version</strong>: {esc(e.get("version",""))}</li>
    <li><strong>last_updated</strong>: {esc(e.get("last_updated",""))}</li>
    <li><strong>viz1</strong>: {esc(e.get("viz1", DEFAULT_VIZ1))}</li>
    <li><strong>viz2</strong>: {esc(e.get("viz2", DEFAULT_VIZ2))}</li>
  </ul>

  {("<h3>Functions</h3><ul>"+func_html+"</ul>") if funcs else ""}
</body>
</html>
"""
